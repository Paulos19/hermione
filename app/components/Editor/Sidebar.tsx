...
export default function Sidebar({ 
  ...
  if (!isOpen) {
    return (
      <aside className="hidden md:flex w-[48px] bg-[var(--theme-bg-surface)] border-r border-[var(--theme-border-subtle)] flex-col h-full shrink-0 items-center py-4 z-30">
        <button 
          onClick={() => setIsOpen(true)}
          className="p-1.5 hover:bg-[var(--theme-bg-surface-elevated)] rounded-md transition-colors text-[var(--theme-text-muted)]"
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 w-[280px] md:relative md:w-[320px] bg-[var(--theme-bg-surface)] border-r border-[var(--theme-border-subtle)] flex flex-col h-full shrink-0 text-[var(--theme-text-main)] select-none transition-all duration-300 shadow-2xl md:shadow-none">
        
        {/* Search Inside Book & Toggle Header */}
        <div className="p-4 border-b border-[var(--theme-border-subtle)] flex items-center gap-2">
          <form onSubmit={handleSearch} className="relative flex-1">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--theme-text-muted)]" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Find in book..."
              className="w-full h-[32px] bg-[var(--theme-bg-surface-elevated)] border border-[var(--theme-border-subtle)] rounded-xl pl-8 pr-3 text-xs text-[var(--theme-text-main)] placeholder-[var(--theme-text-muted)] focus:outline-none focus:border-[#B899FF] transition-all"
            />
          </form>
          <button 
            onClick={() => setIsOpen(false)}
            className="p-1.5 hover:bg-[var(--theme-bg-surface-elevated)] rounded-lg transition-colors text-[var(--theme-text-muted)]"
        ...
        <div className="flex-1 overflow-y-auto py-2 custom-scrollbar">
          ...
          <div className="px-2 mb-6">
            <div 
              className="flex items-center justify-between px-2 py-1.5 cursor-pointer hover:bg-[var(--theme-bg-surface-elevated)] rounded-lg group"
              onClick={() => setIsTreeExpanded(!isTreeExpanded)}
            >
              <div className="flex items-center gap-2 text-xs font-semibold text-[var(--theme-text-muted)] tracking-wider uppercase">
              ...
              <button 
                ...
                className="opacity-0 group-hover:opacity-100 p-1 hover:bg-[var(--theme-bg-surface-elevated)] rounded transition-all text-[var(--theme-text-main)]"
              >
                  ...
                  return (
                    <div
                      ...
                      className={`group flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs cursor-pointer transition-all duration-150 ${
                        isActive 
                          ? "bg-[var(--theme-bg-surface-elevated)] text-[var(--theme-accent)] font-semibold border border-violet-200 dark:border-violet-500/20 shadow-sm" 
                          : "text-[var(--theme-text-muted)] hover:text-gray-900 dark:hover:text-[#F5F5F5] hover:bg-[var(--theme-bg-surface-elevated)]"
                      }`}
                    >
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-1 flex-1">
                          <input
                            ...
                            className="flex-1 bg-[var(--theme-bg-surface-elevated)] border border-violet-500 rounded px-1.5 py-0.5 text-xs text-[var(--theme-text-main)] outline-none"
                            autoFocus
                            onKeyDown={(e) => {
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100">
                            <button
                              ...
                              className="p-1 hover:bg-[var(--theme-border-subtle)] rounded"
                              title="Renomear"
                            >
              ...
              <div className="mt-1 pl-2 space-y-0.5">
                ...
                {characters.map(char => (
                  <div
                    ...
                    className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs cursor-pointer text-[var(--theme-text-muted)] hover:text-gray-900 dark:hover:text-[#F5F5F5] hover:bg-[var(--theme-bg-surface-elevated)]"
                  >
          ...
          <div className="px-2">
            <div 
              className="flex items-center justify-between px-2 py-1.5 cursor-pointer hover:bg-[var(--theme-bg-surface-elevated)] rounded-lg"
            ...
            {isAnalyticsExpanded && (
              <div className="mt-2 px-3 py-3 bg-[var(--theme-bg-surface-elevated)] rounded-xl space-y-3 text-xs text-[var(--theme-text-muted)]">
                ...
                <div>
                  ...
                  <div className="w-full h-1.5 bg-[var(--theme-bg-surface-elevated)] rounded-full overflow-hidden">
                    <div 
                ...
                <div className="w-full h-px bg-[var(--theme-bg-surface-elevated)] my-2" />

                <div className="flex justify-between items-center">
                  <span>{t.wordCount}</span>
                  <span className="font-semibold text-[var(--theme-text-main)]">{wordCount}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span>{t.readingTime}</span>
                  <span className="font-semibold text-[var(--theme-text-main)]">{readingTime}</span>
                </div>
              </div>
            )}
          </div>
