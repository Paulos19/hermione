            ...
            <div className="max-w-[1600px] mx-auto px-4 sm:px-8 md:px-12 py-6 md:py-10 space-y-6 md:space-y-10 pb-32">
              ...
              <section className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="relative w-full sm:max-w-md">
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-[#141A22] border border-gray-200 dark:border-white/5 rounded-xl pl-10 pr-4 py-2.5 text-[14px] text-gray-900 dark:text-[#F5F5F5] placeholder:text-gray-500 focus:outline-none focus:border-violet-600/50 dark:focus:border-[#B899FF]/50 transition-colors shadow-sm"
                  />
                </div>
                ...
                <div className="relative flex items-center gap-2 w-full sm:w-auto" ref={filterRef}>
                  <button 
                    ...
                    className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-white dark:bg-[#141A22] border ${isFilterOpen ? 'border-violet-500 dark:border-[#B899FF]' : 'border-gray-200 dark:border-white/5'} rounded-xl text-[14px] font-medium text-gray-700 dark:text-[#F5F5F5] hover:bg-gray-50 dark:hover:bg-[#181F28] transition-colors shadow-sm`}
                  >
                  ...
                  {/* Filter Dropdown */}
                  {isFilterOpen && (
                    <div className="absolute top-full mt-2 right-0 w-64 bg-white dark:bg-[#10151B] border border-gray-200 dark:border-white/10 rounded-xl shadow-xl shadow-black/10 py-3 z-50">
                      
                      <div className="px-4 pb-2 mb-2 border-b border-gray-100 dark:border-white/5">
                        <p className="text-[12px] font-semibold text-gray-500 dark:text-[#8A94A0] uppercase tracking-wider flex items-center gap-2 mb-2">
                          <SortDesc className="w-3.5 h-3.5" /> Ordenar Por
                        </p>
                        <select 
                          value={filterSort}
                          onChange={(e) => setFilterSort(e.target.value as any)}
                          className="w-full bg-gray-50 dark:bg-[#141A22] border border-gray-200 dark:border-white/5 rounded-lg px-3 py-2 text-[13px] text-gray-900 dark:text-[#F5F5F5] focus:outline-none focus:border-violet-500"
                        >
              ...
              <section>
                {filteredBooks.length > 0 ? (
                    ...
                    {filteredBooks.map(book => (
                      <div key={book.id} className="group flex flex-col gap-4">
                        <Link href={`/${lang}/editor/${book.id}`}>
                          <div className="aspect-[3/4] bg-white dark:bg-[#141A22] border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden relative transition-all duration-300 group-hover:-translate-y-1.5 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.12)] dark:group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] group-hover:border-violet-500/30">
                            {book.coverImage ? (
                        ...
                        <div className="flex items-start justify-between px-1">
                          <div>
                            <p className="text-[14px] text-gray-900 dark:text-[#F5F5F5] font-medium line-clamp-1 mb-0.5">{book.title}</p>
                            <p className="text-[12px] text-gray-500 dark:text-[#8A94A0] flex items-center gap-1">
                              <Clock className="w-3 h-3" /> Atualizado em {formatDate(book.updatedAt, lang)}
                            </p>
                          </div>
                          <button
                            onClick={(e) => handleDeletar(book.id, e)}
                            className="p-2 text-gray-400 dark:text-[#8A94A0] hover:text-red-500 dark:hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10"
                  ...
                  <div className="py-32 border border-gray-200 dark:border-white/5 border-dashed rounded-[24px] bg-white dark:bg-[#141A22]/50 flex flex-col items-center justify-center text-center shadow-sm">
                    <div className="w-20 h-20 bg-gray-50 dark:bg-[#10151B] rounded-2xl flex items-center justify-center mb-6 border border-gray-200 dark:border-white/5 shadow-inner">
                      <BookOpen className="w-10 h-10 text-gray-400 dark:text-[#8A94A0]" />
                    </div>
                    <h3 className="text-[22px] font-medium text-gray-900 dark:text-white mb-3">Nenhum projeto encontrado</h3>
